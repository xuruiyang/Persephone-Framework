...
class ITE(Formula):
	...
	def traverseFormula(self, preorder):
		...
		if self._i_f_type.eval()==1:
			preorder = self._then_form.traverseFormula(preorder)
		else:
			preorder = self._else_form.traverseFormula(preorder)
		return preorder
...
def generatePredicate(name, params, body):
	...
	for p in params:
		params_def += """
		{0}=self._args[{1}]
		self.paramVec[{1}] = self._args[{1}].eval() if not isinstance(self._args[{1}],int) else self._args[{1}]
		# print(self._args[{1}])
		""".format(p,i)
		i+=1
...
def init_statePreorder(formula):
	max_preorder = formula.traverseFormula(0)
	# print('max preorder:',max_preorder)
	return max_preorder

predicate_id = 0
# generatePredicate('HSR', ['k','q','n'],"ITE(EQ(n,1),TrueConst(),ITE(Alg_Or(EQ(k,0),EQ(q,0)),FalseConst(),Exists(m,And(HSR(Sub(k,1),Sub(q,1),m),HSR(k,Sub(q,1),Sub(n,m))),Domain(1,n))))")
# generatePredicate('HSR_COMP', ['k','q'],"Exists(n,And(HSR(k,q,n),Forall(n1,Not(HSR(k,q,n1)),Domain(Add(n,1),20))),Domain(1,20))")

# hsr=HSR_COMP(4,4)
# print(init_statePreorder(self.definition()))
# game = SemanticGame(hsr,Player(Role_P(),'P1'),Player(Role_OP(),'P2'))
# game.play()