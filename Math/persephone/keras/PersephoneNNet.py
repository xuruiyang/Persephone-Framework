...
class PersephoneNNet():
    def __init__(self, game, args):
        ...
        stop_grad_vs_fc2 = Lambda(lambda x: K.stop_gradient(x))(vs_fc2)

        self.concept =  Dense(1, activation='sigmoid', name='concept', kernel_regularizer=regularizers.l1(0.05), bias_regularizer=regularizers.l1(0.05))(stop_grad_vs_fc2)
        ...
        "v": "mean_squared_error",
        "concept": "mean_squared_error"}

        self.model = Model(inputs=[self.input_boards, adv, old_prediction], outputs=[self.pi, self.v, self.concept])
        self.model.compile(loss=loss_funcs, optimizer=Adam(args.lr))

        self.predict = Model(inputs=self.input_boards, outputs=[self.pi, self.v, self.concept])